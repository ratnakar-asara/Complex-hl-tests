...
func PostUser(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if err != nil {
		...
		keys := []string{args[0]}
		err = CUD_UserRecord(stub, "UserTable", keys, buff)
		if err != nil {
...
func CUD_UserRecord(stub shim.ChaincodeStubInterface, tableName string, keys []string, args []byte) error {
	...
	if err != nil {
		...
		if err != nil {
			fmt.Printf("\nCUD_UserRecord: UpdateRow into "+tableName+" Table operation failed. %s\n", err)
			return errors.New("CUD_UserRecord: UpdateRow into "+tableName+" Table operation failed. ")
		}
		if !ok {
			fmt.Printf("\nCUD_UserRecord: UpdateRow into "+tableName+" Table operation failed. %s\n", err)
			return errors.New("CUD_UserRecord: UpdateRow into "+tableName+" Table operation failed.")
	...
	if !ok {
		...
		if !ok {
			fmt.Printf("\nCUD_UserRecord: InsertRow into "+tableName+" Table operation failed. %s\n", err)
			return errors.New("CUD_UserRecord: InsertRow into "+tableName+" Table operation failed.")
		}
	}
	fmt.Printf("\nCUD_UserRecord: InsertRow/UpdateRow into "+tableName+" Table operation Successful")
	/*ok, err = stub.ReplaceRow(tableName, row)
	if err != nil {
		fmt.Printf("\nCUD_UserRecord: UpdateRow into "+tableName+" Table operation failed. %s\n", err)
		return errors.New("CUD_UserRecord: UpdateRow into "+tableName+" Table operation failed. ")
	...
	if !ok {
		return errors.New("CUD_UserRecord: UpdateRow into " + tableName + " Table failed.")
	}
	fmt.Printf("\nCUD_UserRecord: UpdateRow into "+tableName+" Table operation Successful")
	var column []shim.Column
	colNext := shim.Column{Value: &shim.Column_String_{String_: keys[0]}}
	column = append(column, colNext)

	//colNext := shim.Column{Value: &shim.Column_String_{String_: string(args[0])}}
	//column = append(column, colNext)
	//func (stub *ChaincodeStub) DeleteRow(tableName string, key []Column) error {
	err = stub.DeleteRow(tableName, column)
	if err != nil {
		fmt.Printf("\nCUD_UserRecord: DeleteRow on "+tableName+" Table operation failed. %s\n", err)
		return errors.New("CUD_UserRecord: DeleteRow on "+tableName+" Table operation failed.")
	}
	fmt.Printf("\nCUD_UserRecord: Deletion of the record from "+tableName+" Table is successful\n\n")
	fmt.Println("CUD_UserRecord: Create/Update/Deletion of Row on ", tableName, " Table operation Successful. ")*/
	return nil
}
